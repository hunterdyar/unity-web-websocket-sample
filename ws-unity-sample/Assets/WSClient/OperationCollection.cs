...
namespace Clayze
{
	...
	public class OperationCollection : ScriptableObject
	{
		...
		private void RemoveDataFromServer(byte[] data)
		{
		...
		private void AddOperationsFromServer(byte[] data)
		{
			...
			{
				consumed = AddDataFromServer(data,offset,false);
				offset += consumed;
				if (consumed == 0)
				{
					break;//uh oh!
		...
		/// </summary>
		public int AddDataFromServer(byte[] data, int startOffset, bool sendUpdate = true)
		{
